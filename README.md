...
Codex Switch 是一个本地桌面中转工具. 它提供 OpenAI 和 Anthropic 兼容代理, 上游账号管理, 模型调度, 请求统计和运行状态查看, 适合在一台个人电脑上集中管理 Codex OAuth 账号和 API Key 上游.

应用使用 Rust, eframe/egui 和 SQLite 构建. 默认只监听 `127.0.0.1:15721`. 代理服务默认需要在仪表盘手动启动, 也可以勾选 "启动时启用服务".
...
| 系统 | x64 | arm64 |
| --- | --- | --- |
| Windows | `codex-switch-<version>-windows-x86_64.zip` | `codex-switch-<version>-windows-aarch64.zip` |
| Linux | `codex-switch-<version>-linux-x86_64.tar.gz` | `codex-switch-<version>-linux-aarch64.tar.gz` |
| macOS | `codex-switch-<version>-macos-x86_64.dmg` | `codex-switch-<version>-macos-aarch64.dmg` |
...
本地访问 key 可以通过 Bearer 或 `x-api-key` 发送. 可以用下面的请求确认模型接口已经可用:

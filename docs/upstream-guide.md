...
| 余额/额度显示 | 同一个上游既有额度窗口又有金额余额时决定显示哪一份, 见[查询套餐额度](#查询套餐额度) |
...
- API Key, 余额 provider, 余额与额度显示方式, 余额自动刷新, 余额提醒和缓存保持.
...
上游编辑器的 `余额/额度显示` 决定界面和托盘显示哪一份数据, 因为同一个上游可能既有套餐额度窗口又有金额余额 (例如智谱):

| 取值 | 说明 |
| --- | --- |
| `自动` | 有额度窗口就显示窗口, 没有才显示金额余额 |
| `额度窗口 42%/49%/60%` | 始终显示额度窗口并带百分号 |
| `额度窗口 42/49/60` | 始终显示额度窗口, 不带百分号 |
| `金额余额` | 始终显示金额余额, 忽略额度窗口 |

`自动` 是默认值. 托盘空间有限, 因此托盘标题用 `42/49/60` 紧凑形式, 上游列表和仪表盘用 `42%/49%/60%` 完整形式, 显式选择某一种形式后两处都会跟随. 上游没有任何窗口数据时, 无论选哪种取值都回落到金额余额.
...
托盘和上游列表都可以显示这些窗口, 见[界面说明](app-guide.md).

...
`为非多模态模型去除多模态输入` 只在上游编辑页中提供. 开启后, Codex Switch 会优先使用本会话内模型列表接口返回的能力缓存, 未命中时读取已落盘的 models.dev 模型信息兜底, 仍未命中时按该上游的未知模态配置处理, 不再依赖内置模型名单. 被判定为文本模型时, 转发前会把图片, 音频和文件替换为说明模型不支持对应输入, 并包含媒体类型, 大小, 文件名或尺寸的文字描述, 其他文本消息保持不变. 远程媒体没有内嵌数据时, 描述中只标注远程来源. 该设置不会改写请求模型本身, 模型列表实时缓存重启应用后失效, models.dev 能力兜底会随模型信息缓存持久保存.
